...
**说明**：Go Backend 和 LiteLLM Gateway 为同层级服务，并列服务于 Web UI 和 SDK 客户端。

### 2.2 架构分层

| 层级 | 组件 | 职责 | 技术选型 |
|------|------|------|----------|
| **接入层** | Web UI / SDK / OpenAI SDK | 客户端访问 | 浏览器 / CLI / SDK |
| **同层级服务** | Go Backend + LiteLLM Gateway | 管理面 + 推理面 | Go + Gin / LiteLLM |
| **执行层** | Dynamo | 实际 LLM 推理执行 | Dynamo + vLLM/SGLang |
| **数据层** | PostgreSQL + Redis | 持久化、缓存、消息队列 | PostgreSQL + Redis |

**设计说明**：移除 API Gateway，Go Backend 和 LiteLLM Gateway 为同层级服务：
- **Go Backend**：管理面 API (:8001)，负责认证、鉴权、模型/部署/租户管理
- **LiteLLM Gateway**：推理面 API (:4000)，负责 OpenAI 兼容 API、多模型路由、限流
...
| **LLM 网关** | LiteLLM | main | 统一 API、OpenAI 兼容 |
...
│  │                                                                      │  │
│  │  ┌────────────────┐  ┌────────────────┐  ┌────────────────┐       │  │
│  │  │ Go Backend     │  │ LiteLLM       │  │ Worker         │       │  │
│  │  │ (3 replicas)    │  │ Gateway       │  │ (2 replicas)   │       │  │
│  │  │ :8001           │  │ (3 replicas)  │  │                │       │  │
│  │  └───────┬────────┘  └───────┬────────┘  └───────┬────────┘       │  │
│  │          │                    │                    │                 │  │
│  │          └────────────────────┴────────────────────┘                │  │
